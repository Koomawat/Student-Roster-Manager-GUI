...
public class Roster {
     ...
     *
     * @return the student roster by no specific order
     */
    public String print() {
        String printing;
     ...
     *
     * @return the student roster by name.
     */
    public String printByName() {
        String printing;
        printing = "* list of students ordered by name **\n";

        Student[] rosterCopy = new Student[getSize()];

        for (int i = 0; i < getSize(); i++) {
            rosterCopy[i] = getStudentRoster()[i];
        }

        for (int i = 0; i < getSize() - Constants.ROSTER_SIZE_CHANGE; i++) {
            for (int j = i + Constants.ROSTER_SIZE_CHANGE; j < getSize(); j++) {
                if (rosterCopy[i].getProfile().getName().compareTo(getStudentRoster()[j].getProfile().getName()) > 0) {
                    Student temp = rosterCopy[i];
                    rosterCopy[i] = rosterCopy[j];
                    rosterCopy[j] = temp;
                }
            }
        }
        ...
        for (int i = 0; i < getSize(); i++) {
            String data = rosterCopy[i].toString();
            printing += data + "\n";
        }
     ...
     *
     * @return the student roster by payment date.
     */
